  ...
  <div class="picker-class fixed-b">
    <div class="picker-box" :class="{'ease-in': showPicker, 'ease-out': !showPicker}">
      ...
      <picker-view class="picker-view" indicator-class="picker-indicator" :value="[index]" @change="pickerChange" >
        <picker-view-column>
          <div class="city-item" v-for="(item, index) in list" :key="index">{{item.childName}}</div>
        </picker-view-column>
      </picker-view>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    sure () {
      this.cancel()
      setTimeout(() => {
        this.$emit('sure', { value: this.list[this.idx], index: this.idx })
      }, 300)
    },
    pickerChange (e) {
